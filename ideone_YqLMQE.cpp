#include<iostream>
 ...
 class link{ //link list class has been declare which contains head and tail(two node pointer)
 	...
 	/* declaring a bunch of function which is associated with link list class*/
 ...
 int  main(){
 	cout<<" program to implement doubly linked list"<<endl;
 ...
 void link::dlt(int posi){ //function to delete at a position 
 	if(posi==count){ // if the last element is said to delete
 		...
 		delete pos;
 			...
 			count--; //as a node has been deleted so the count will be one less
		}
